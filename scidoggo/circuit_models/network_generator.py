...
def create_nx_plot(
    ...
    for idx in inpidx:
        pos[idx] = (
            np.array([np.cos(idx * 2 * np.pi / len(inpidx)), np.sin(idx * 2 * np.pi / len(inpidx))])
            * inp_pos
        )

    for idx in recidx:
        pos[idx] = (
            np.array([np.cos(idx * 2 * np.pi / len(recidx)), np.sin(idx * 2 * np.pi / len(recidx))])
            * rec_pos
        )
    ...
    for idx in inpidx:
        nx.draw_networkx_nodes(
            ...
            pos,
            nodelist=[idx],
            node_color=(iinh_color if idx in inhidx else iexc_color),
            node_size=700 * node_scale,
            ax=ax,
            edgecolors=(highlight_color if idx in highlight_idcs else None),
        )
    ...
    for idx in recidx:
        nx.draw_networkx_nodes(
            ...
            pos,
            nodelist=[idx],
            node_color=(rinh_color if idx in inhidx else rexc_color),
            node_size=700 * node_scale,
            ax=ax,
            edgecolors=(highlight_color if idx in highlight_idcs else None),
        )
    ...
    for u, v, d in G.edges(data=True):
        nx.draw_networkx_edges(
            ...
            edge_color=(
                iinh_color
                if u in inpidx and u in inhidx
                else iexc_color
                if u in inpidx
                else rinh_color
                if u in inhidx
    ...
    if annot:
        labels = {idx: str(idx) for idx in inpidx.tolist() + recidx.tolist()}
